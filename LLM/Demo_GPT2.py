...
def generate_answer_with_huggingface(query):
    docs = hybrid_retrieve_from_pinecone_docs(query)
    #print("This is the docs: ", docs)
    context = "\n".join(docs)
    print("This is the context: ", context)
    #context = context[:800]
    
    # Combine context and query for Hugging Face model prompt
    prompt = (
        "You are a highly knowledgeable assistant with access to a range of documents about Occidental College. "
        "Use the following documents to answer the question accurately. Please talk about and summarize all the documents brielfy. Please refer to the given information when making statement:\n\n"
        f"{context}\n\n"
        f"Question: {query}\nAnswer:"
    )
...
# Test the function
generate_answer_with_huggingface("What are some internship opportunities at Occidental College?")
...
# generate_answer_with_chatgpt("What are some internship opportunities at Occidental College?")
